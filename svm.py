...
def rbf(x_1, x_2, sigma=1.):
    ...
    num_test=x_1.shape[0]
    
    L2_distances_square=np.reshape(np.sum(x_1**2, axis=1), [num_test,1]) + \
    np.sum((x_2)**2, axis=1) - 2 * x_1@(x_2.T)
    distances = np.exp(-L2_distances_square/(sigma**2)/2) ### YOUR CODE HERE
